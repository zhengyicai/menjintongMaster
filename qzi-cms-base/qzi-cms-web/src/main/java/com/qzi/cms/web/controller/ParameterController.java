...
public class ParameterController {
	...
	@Resource
	private AdminSumService adminSumService;
	...
	public RespBody adminFindAll(Paging paging){
		RespBody respBody = new RespBody();
		try {
			//保存返回数据
			respBody.add(RespCodeEnum.SUCCESS.getCode(), "参数设置查找所有数据成功", adminSumService.findAll(paging));
			//保存分页对象
			paging.setTotalCount(adminSumService.findCount());
			respBody.setPage(paging);
		} catch (Exception ex) {
			respBody.add(RespCodeEnum.ERROR.getCode(), "参数设置查找所有数据失败");
			LogUtils.error("参数设置查找所有数据失败！",ex);
