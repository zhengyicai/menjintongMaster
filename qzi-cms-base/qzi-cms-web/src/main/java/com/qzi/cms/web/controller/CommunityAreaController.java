...
public class CommunityAreaController {
	...
	public RespBody findAll(Paging paging,String criteria){
		...
		try {
			//保存返回数据
			respBody.add(RespCodeEnum.SUCCESS.getCode(), "查找所有住宅小区数据成功", communityService.findAll(paging,criteria));
			//保存分页对象
			paging.setTotalCount(communityService.findCount(criteria));
	...
	@PostMapping("/trueDelete")
	public RespBody trueDelete(@RequestBody UseCommunityVo useCommunityVo){
		RespBody respBody = new RespBody();
		try {
			useCommunityMapper.updateDelete(useCommunityVo.getState(),useCommunityVo.getId());
			useCommunityMapper.updateUser(useCommunityVo.getState(),useCommunityVo.getSysUserId());
			useCommunityMapper.updateResident(useCommunityVo.getState(),useCommunityVo.getId());
			useCommunityMapper.updateEquipment(useCommunityVo.getState(),useCommunityVo.getId());
